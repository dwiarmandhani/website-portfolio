...
class Dashboard extends CI_Controller
{
	...
	public function pengaturan_update()
	{
		...
		$this->form_validation->set_rules('aboutdesc', 'About', 'required');
		...
		if ($this->form_validation->run() != false) {
			...
			$aboutdesc = $this->input->post('aboutdesc');
			...
			if (!empty($_FILES['bg_intro']['name'])) {
				...
				$config['upload_path']   = './assets_frontend/img/';
				$config['allowed_types'] = 'jpg|png';

				$this->load->library('upload', $config);

				if ($this->upload->do_upload('bg_intro')) {
					// mengambil data tentang gambar logo yang diupload
					$gambar = $this->upload->data();

					$bgintro = $gambar['file_name'];

					$this->db->query("UPDATE pengaturan SET bg_intro='$bgintro'");
				}
			}
			if (!empty($_FILES['profilepict']['name'])) {
				...
				if ($this->upload->do_upload('profilepict')) {
					...
					$profilepict = $gambar['file_name'];

					$this->db->query("UPDATE pengaturan SET profilepict='$profilepict'");
				}
			}
	...
	{
		$data['layanan'] = $this->m_data->get_data('layanan')->result();
		$this->load->view('dashboard/v_header');
	...
	{
		$url = "";
		if ($redirectTo === "whatssApp") {
			$url = "https://wa.me/6285721813979";
		} else if ($redirectTo === "linkedin") {
			$url = "https://www.linkedin.com/in/dwiarmandhani";
		}
		header("Location: " . $url);
		exit;
	}
